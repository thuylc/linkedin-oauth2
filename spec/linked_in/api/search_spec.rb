...
describe LinkedInOauth2::Search, helpers: :api do
  ...
  it "lets you search for all your contacts" do
    ...
    expect(api.search()).to be_kind_of LinkedInOauth2::Mash
  end

  it "allows a keyword search with no hash" do
    stub("https://api.linkedin.com/v1/people-search?keywords=Proximate%20Harvard&")
    expect(api.search("Proximate Harvard")).to be_kind_of LinkedInOauth2::Mash
  end

  it "allows a keyword search with the kewywords option" do
    stub("https://api.linkedin.com/v1/people-search?keywords=Proximate%20Harvard&")
    expect(api.search(keywords: "Proximate Harvard")).to be_kind_of LinkedInOauth2::Mash
  end
  ...
  it "lets you search by an attribute" do
    ...
    expect(api.search(school_name: "Olin College of Engineering")).to be_kind_of LinkedInOauth2::Mash
  end

  it "combines searches" do
    stub("https://api.linkedin.com/v1/people-search?first-name=Evan&last-name=Morikawa&")
    expect(api.search(first_name: "Evan", last_name: "Morikawa")).to be_kind_of LinkedInOauth2::Mash
  end
  ...
  it "searches for specific fields" do
    ...
    expect(api.search(fields: [{people: ["id", "first-name", "last-name"]}, "num-results"])).to be_kind_of LinkedInOauth2::Mash
  end

  it "allows you to pass a sort parameter" do
    stub("https://api.linkedin.com/v1/people-search?sort=connections&")
    expect(api.search(sort: "connections")).to be_kind_of LinkedInOauth2::Mash
  end

  it "it allows start and count parameters" do
    stub("https://api.linkedin.com/v1/people-search?start=10&count=5&")
    expect(api.search(start: 10, count: 5)).to be_kind_of LinkedInOauth2::Mash
  end
  ...
  it "it lets you search by facets" do
    ...
    expect(api.search(fields: fields,
                      facets: "location")).to be_kind_of LinkedInOauth2::Mash
  end
  ...
  it "lets you search for complex facets" do
    ...
    expect(api.search(fields: fields,
          ...
          ).to be_kind_of LinkedInOauth2::Mash
  end
end
